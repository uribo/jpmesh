...
meshcode_set <- function(mesh_size = c("80km", "10km", "1km")) {
  mesh_sets_vector <- as.character(c(3036,
  ...
  mesh_set <- dplyr::case_when(
    mesh_size == "80km" ~ rlang::as_list(mesh_sets_vector),
    mesh_size == "10km" ~ mesh_sets_vector %>% 
      purrr::map(fine_separate)
  )
  if (mesh_size == "1km") {
    mesh_set <- mesh_sets_vector %>% 
      purrr::map(fine_separate) %>% 
      purrr::reduce(c) %>% 
      purrr::map(fine_separate)
  }
  
  mesh_set <- mesh_set %>% purrr::as_vector()
